...
export const verifyWorkProgress = async (progressId, verificationNotes = '', approved = true) => {
  try {
    ...
    if (approved && data.progress_type === 'completion') {
      ...
      if (tenderError) console.error('Error updating tender status:', tenderError);
      ...
      const { error: issueError } = await supabase
        ...
        .eq('id', (
          SELECT source_issue_id FROM tenders WHERE id = data.tender_id
        ));

      if (issueError) console.error('Error updating issue status:', issueError);
    }
